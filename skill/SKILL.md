...
The injected CMake block walks user directories with `CONFIGURE_DEPENDS` for `.c`, `.cc`, `.cpp`, `.cxx`, `.s`, `.S`, `.asm`, `.h`, `.hh`, `.hpp`, and `.hxx` files. It never enters the active `CMAKE_BINARY_DIR`, generated trees, or common test/example/tool trees, and it registers the root `.ioc` as a configure dependency. Exclusions are case-insensitive directory names relative to the project root, so a parent folder named `tests` does not hide the whole project and custom names cannot inject regex syntax. Override the `QODER_SOURCE_EXCLUDE_DIRS` cache variable when a project needs a different policy. The next Build therefore rechecks new files and CubeMX changes. The hook invokes the project-local `generate_vscode.ps1` during Configure and passes the actual `CMAKE_BINARY_DIR`, so custom preset output directories remain aligned with the generated ELF path.
...
`generate_vscode.ps1` maintains only the named ST-Link and DAPLink entries. It preserves AmphiLink CFG Tool and other custom `launch.json` entries, preserves unrelated tasks, and refuses to overwrite JSONC or malformed files. For an AmphiLink entry (identified by its name or `amphilink-cfg-openocd.cfg`), it adds `preLaunchTask: CMake Build` and `set remotetimeout 10` when those fields are absent. This keeps F5 on the current ELF and tolerates normal wireless GDB idle latency. If the AmphiLink extension rewrites its managed entry after saving, the next CMake Configure reapplies these fields. If tool discovery is incomplete, retain an existing complete DAPLink entry instead of replacing it with a partial one.
